/**
 * @class
 * @name vcui.ui.SmoothScroll
 * @extends vcui.ui.View
 * @description SmoothScroll -benchmark : iScroll v5.1.2 ~
 * @version 1.0
 * 
 */
...
vcui.define('ui/smoothScroll', ['jquery', 'vcui'], function ($, core) {
    ...
    var SmoothScroll = core.ui('SmoothScroll', /** @lends vcui.ui.SmoothScroll# */{
         ...
         * @param {number} [options.startX = 0] x축 시작값
         * @param {number} [options.startY = 0] y축 시작값
         * @param {boolean} [options.scrollX = true] x측 스크롤 유무
         * @param {boolean} [options.scrollY = true] y측 스크롤 유무
         * @param {number} [options.directionLockThreshold = 5] 잠김 임계값
         * @param {number} [options.mouseWheelSpeed = 20] 휠 스피드
         * @param {boolean} [options.momentum = true] 저항에니메이션 유무
         * @param {boolean} [options.autoCenterScroll = true] 자동 센터 스크롤
         * @param {string} [options.prevButton = ''] 이전버튼 선택자
         * @param {string} [options.nextButton = ''] 다음버튼 선택자
         * @param {boolean} [options.bounce = true] 바운스 애니메이션
         * @param {number} [options.bounceTime = 600] 바운스 애니메이션 시간
         * @param {string} [options.bounceEasing = ''] 바운스 애니메이션 타입 quadratic | circular | back | bounce | elastic
         * @param {boolean} [options.preventDefault = false] 이벤트 제어
         * @param {object} [options.preventDefaultException = {tagName : /^(INPUT|TEXTAREA|BUTTON|SELECT)$/i }] 이벤트 제어 예외 태그
         * @param {boolean} [options.HWCompositing = true] 하드웨어 가속
         * @param {boolean} [options.useTransition = true] transtion css 속성사용
         * @param {boolean} [options.useTransform = true] transform css 속성사용
         * @param {boolean} [options.resizeRefresh = true] 리사이즈이벤트 새로고침 
         */   
