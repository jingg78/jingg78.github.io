/**
 * @class
 * @name vcui.ui.Carousel
 * @description Carousel 컴포넌트
...
vcui.define('ui/carousel', ['jquery', 'vcui','libs/jquery.transit'], function ($, core) {
    "use strict";
    ...
    var Carousel = core.ui('Carousel', /**@lends vcui.ui.Carousel# */{
        bindjQuery: _N,
        defaults: {
         ...
         * @param {boolean} [options.dots = true]  인디케이터 사용 여부
         * @param {boolean} [options.infinite = false] 무한루프 사용 여부
         * @param {boolean} [options.swipe = true] 스와이핑 허용 여부
         * @param {boolean} [options.variableWidth = false] 가변 너비 슬라이드
         * @param {number} [options.speed = 800] 슬라이딩 속도(0.8초)
         * @param {number} [options.slidesToShow = 1] 표시할 슬라이드 수
         * @param {number} [options.slidesToScroll = 1] 슬라이딩될 때 한번에 움직일 갯수
         * @param {object} [options.responsive = null] 브레이크포인트에 따른 설정값들  
         */
         ...
         * slide를 이동
         * @param {object} event 
         * @param {string} [event.message = "index"]   이벤트 타입
         * @param {number} [event.index = 0]  인텍스로 이동
         * @param {boolean} [dontAnimate=false] 애니메이션 유무      
         * @example
         * $('...').vcCarousel('changeSlide', {message: 'index', index: 0}, true);
         */
        ...
        getLeft: function getLeft(slideIndex) {
            ...
            if (opt.variableWidth === true) {
                ...
                if (opt.rtl === true) {
                    ...
                    // dududu1 variableWidth == true 설정후 slide 다른 영역을 일부 보여주는 형식일때 사용
                    if(opt.infinite === true){
