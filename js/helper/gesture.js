 ...
 * self.$el.vcGesture({
    ...
    }).on('gesturestart gesturemove gestureend gesturecancel', function (e, data) {
        switch(e.type) {
            case 'gesturestart':
                break;
            default:
...
vcui.define('helper/gesture', ['jquery', 'vcui'], function($, core) {
    ...
    /**
     * 발생되는 이벤트
     * @fires vcui.helper.Gesture#gesturestart
     * @fires vcui.helper.Gesture#gesturemove
     * @fires vcui.helper.Gesture#gestureend
     * @fires vcui.helper.Gesture#gesturecancel
     */

    var Gesture = core.helper('Gesture', core.ui.View.extend(/** @lends vcui.helper.Gesture# */{
        name: 'Gesture',
        defaults: {
         ...
         * @param {element} [options.container = document] move이벤트를 어디에 걸 것인가
         * @param {number} [options.threshold = 50] 움직임을 감지하기 위한 최소 크기
         * @param {string} [options.direction = 'horizontal'] 'vertical', 'both' 중 택일
         * @param {function} [options.gesture = null] 모든 제스처에 의해 호출되는 콜백형식의 헨들러함수. 이벤트 랑 콜백방식 중에 아무거나 편한거 사용
         * @param {function} [options.gestureStart = null] 터치가 시작될 때 호출되는 핸들러. 이벤트 랑 콜백방식 중에 아무거나 편한거 사용
         * @param {function} [options.gestureMove = null] 터치가 움직일 때 호출되는 핸들러. 이벤트 랑 콜백방식 중에 아무거나 편한거 사용
         * @param {function} [options.gestureEnd = null] 터치가 끝났을 때 호출되는 핸들러. 이벤트 랑 콜백방식 중에 아무거나 편한거 사용
         */
                ...
                $container.on(makeEventNS('mousemove touchmove', eventNS), function(moveEvent) {
                    ...
                    if (!isSwipe) {
                        ...
                        if (swipeX < 1 && self.isHoriz || swipeY < 1 && self.isVerti) {
                            ...
                            /**
                             * @event vcui.helper.Gesture#gesturestart
                             */
                    ...
                    if (isSwipe) {
                        ...
                        /**
                         * @event vcui.helper.Gesture#gesturemove
                         */
                    ...
                    if (isSwipe && touchStart) {
                        ...
                        if (Math.abs(touch.diff.x) > opt.threshold || Math.abs(touch.diff.y) > opt.threshold) {
                            ...
                            /**
                             * @event vcui.helper.Gesture#gestureend
                             */
                            self.triggerHandler('gestureend', touch);
                        } else {
                            self._gestureCallback('cancel', touch);
                            /**
                             * @event vcui.helper.Gesture#gesturecancel
                             */
